...
Every modern web applications built with PHP now requires a huge amount of logic handled at the frontend to enhance and provide awesome user experience. PHP frameworks such as [Laravel](https://laravel.com/) comes bundled with support for crafting the client side logic using [Vue.js](https://vuejs.org/). This amongst other things gives developers the ability to rapidly build applications by combining these technologies.
